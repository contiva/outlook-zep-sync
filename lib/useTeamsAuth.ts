...
export function useTeamsAuth(): TeamsAuthState {
  ...
  const performTeamsAuth = useCallback(async () => {
    ...
    try {
      ...
      const contextName =
        context.user?.displayName || context.user?.userPrincipalName?.split('@')[0] || null;
      ...
      import('@/lib/teams-sdk')
        .then((sdk) => sdk.notifyAppLoadFailed(errorMessage))
        .catch(() => {});
