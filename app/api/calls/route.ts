...
function resolveName(identity: Identity | Participant | OrganizerIdentity | null | undefined): string {
  ...
  if ("application" in identity && identity.application) return identity.application.displayName || "App";
  return "Unknown";
}
...
async function buildUserMappings(accessToken: string): Promise<void> {
  ...
  while (userUrl) {
    ...
    for (const u of data.value) {
      ...
      if (u.mobilePhone) {
        phoneVariants(u.mobilePhone).forEach((v) =>
          phoneToUser.set(v, u.displayName)
        );
      }
      if (u.businessPhones) {
...
async function getScheduledMeetingUrls(
  ...
  let calUrl: string | null = `https://graph.microsoft.com/v1.0/users/${userId}/calendarView?startDateTime=${startDate}T00:00:00Z&endDateTime=${endDate}T23:59:59Z&$select=isOnlineMeeting,onlineMeeting&$top=250`;
...
async function getCallRecordIds(
  ...
  let callUrl: string | null = `https://graph.microsoft.com/v1.0/communications/callRecords?$filter=startDateTime ge ${startDate}T00:00:00Z and startDateTime le ${endDate}T23:59:59Z`;
...
export async function GET(request: Request): Promise<NextResponse<CallsResponse | { error: string }>> {
  ...
  if (startDate < minStartDateStr) {
    console.log(`[/api/calls] Adjusting startDate from ${startDate} to ${minStartDateStr} (30-day retention limit)`);
  ...
  try {
    ...
    console.log(`[/api/calls] Found ${callIds.length} call record IDs for ${startDate} to ${endDate}`);
    ...
    return NextResponse.json(
      { error: `Failed to fetch calls: ${errorMessage}` },
      { status: 500 }
    );
  }
}
