...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (employee.userId !== employeeId) {
      console.warn(
        `Security: User ${user.email} (ZEP: ${employee.userId}) tried to access projects for ${employeeId}`,
      );
      return NextResponse.json(
  ...
  try {
    const projekte = await getBookableProjects(token, employeeId, referenceDate || undefined);
