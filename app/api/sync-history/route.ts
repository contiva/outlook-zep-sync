...
async function validateUserAccess(
  ...
  if (employee.userId !== requestedUserId) {
    ...
    return {
      valid: false,
      error: 'Zugriff verweigert: Sie können nur Ihre eigene Sync-Historie abrufen',
      status: 403,
    };
  }
...
export async function GET(request: Request) {
  ...
  try {
    if (outlookEventId) {
      ...
      if (!mapping) {
        return NextResponse.json({ error: 'Sync mapping not found' }, { status: 404 });
      }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to save sync mappings' }, { status: 500 });
  }
}
...
export async function DELETE(request: Request) {
  ...
  if (!userId || !outlookEventId) {
    return NextResponse.json({ error: 'userId and outlookEventId are required' }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to delete sync mapping' }, { status: 500 });
  }
}
